...
public class AlphaConnectionHandler extends SimpleChannelInboundHandler<Packet> {
    ...
    private void handleLogin(ChannelHandlerContext ctx, int loginProtocolVersion) {
        ...
        //
        // ALL v2 clients get SpawnPositionPacketAlphaver with seed=0L. The extra 8
        // zero bytes are harmlessly decoded by standard Alpha 1.1.0 as 8 KeepAlive
        // (0x00) no-ops — KeepAlive has zero payload. Alphaver clients read the seed
        // field properly. We CANNOT gate on alphaverDetected here because the 0x07
        // skin request that triggers detection arrives AFTER the login sequence.
        // Sending the short SpawnPositionPacket to an Alphaver client would cause it
        // to consume 8 bytes from the next packet, misaligning the TCP stream.
