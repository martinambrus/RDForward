...
| EaglerCraft | Alpha 1.2.6, Beta 1.3, Beta 1.7.3, 1.5.2, 1.8.8, 1.12.2 | WebSocket, protocol 6/9/14/61/47/340 |
| Bedrock | 1.26.10 (protocol 944) | UDP, RakNet |

Protocol detection is automatic — the server identifies the client type from the first bytes of the connection and configures the Netty pipeline accordingly. Classic versions are distinguished by the second byte: 0.0.15a sends a username character (no protocol version byte), 0.0.16a-0.0.19a send protocol 3-5 (129-byte identification), 0.0.20a-0.27 send protocol 6 (130-byte identification), and 0.28-0.30 send protocol 7. EaglerCraft clients are detected by the HTTP `GET` request (first byte `0x47`), which triggers a WebSocket upgrade. EaglerCraftX clients use a custom pre-MC handshake before entering standard play — supporting protocol 6 (Alpha 1.2.6), 9 (Beta 1.3), 14 (Beta 1.7.3), 47 (1.8.8), and 340 (1.12.2). EaglerCraft 1.5.2 clients send standard pre-Netty MC packets directly over WebSocket. LCE clients are detected by a server-sends-first handshake (300ms timeout), while all other TCP protocols are identified from the first client byte. Alphaver clients (which report the same protocol version as standard Alpha 1.1.0) are detected reactively after login via a client-specific skin request packet.
...
| [EaglerXServer](https://github.com/lax1dude/eaglerxserver) / [EaglercraftX](https://github.com/AscendStudios/EaglercraftX-1.8.8-src) | Reference for EaglerCraft WebSocket transport and custom pre-MC handshake protocol (V1-V3). Packet formats, MOTD query handling, skin data encoding, and pipeline configuration were derived from the EaglerXServer proxy and EaglercraftX client source. |
