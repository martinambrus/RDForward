...
class CreativeBlockPlacementTest {
    ...
    @Test
    void v340BlockIdsConvertToCobblestone() throws Exception {
        BotClient bot = testServer.createBot(ProtocolVersion.RELEASE_1_12_2, "V340Sweep");
        try {
            BotSession session = bot.getSession();
            assertTrue(session.isLoginComplete(), "Login should complete");
            ...
            for (int i = 0; i < blockIds.length; i++) {
                ...
                int blockType = session.waitForBlockChangeValue(x, 41, z, COBBLESTONE, 3000);
                assertEquals(COBBLESTONE, blockType,
                        "Block ID " + blockIds[i] + " should convert to cobblestone below surface (V340)");
            }
        } finally {
            bot.disconnect();
        }
