...
/**
 * Release 1.20.3: Creative block palette test. Opens creative inventory, grabs items from successive slots, places them in a row on the ground, and captures a screenshot.
 *
 * Run with: ./gradlew :rd-e2e:test -Pe2e
 */
class Release1203CreativeBlockPaletteTest {
    ...
    static void setUp() throws Exception {
        ...
        statusDir = Files.createTempDirectory("e2e-release1203-palette-").toFile();
    ...
    @Test
    void release1203CreativeBlockPalettePasses() throws Exception {
        String agentJar = findAgentJar();
        assertNotNull(agentJar, "rd-e2e-agent fat JAR not found. Run ./gradlew :rd-e2e-agent:fatJar first.");

        Process client = launcher.launchRelease1203(agentJar, server.getPort(),
        ...
        try {
            ...
            new ScreenshotBaselineVerifier("release1203", "creative_block_palette")
