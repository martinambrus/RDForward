...
/**
 * Release 1.20: Creative block palette test. Opens creative inventory, grabs items from successive slots, places them in a row on the ground, and captures a screenshot.
 *
 * Run with: ./gradlew :rd-e2e:test -Pe2e
 */
class Release120CreativeBlockPaletteTest {
    ...
    static void setUp() throws Exception {
        ...
        statusDir = Files.createTempDirectory("e2e-release120-palette-").toFile();
    ...
    @Test
    void release120CreativeBlockPalettePasses() throws Exception {
        String agentJar = findAgentJar();
        assertNotNull(agentJar, "rd-e2e-agent fat JAR not found. Run ./gradlew :rd-e2e-agent:fatJar first.");

        Process client = launcher.launchRelease120(agentJar, server.getPort(),
        ...
        try {
            ...
            new ScreenshotBaselineVerifier("release120", "creative_block_palette")
