...
/**
 * Verifies the spawn environment for Alpha 1.0.17_04: player on grass,
 * clear surroundings, 64 cobblestone.
 *
 * Run with: ./gradlew :rd-e2e:test -Pe2e --tests "*Alpha1017Environment*"
 */
class Alpha1017EnvironmentCheckTest {
    ...
    static void setUp() throws Exception {
        ...
        statusDir = Files.createTempDirectory("e2e-alpha1017-env-").toFile();
        statusDir.deleteOnExit();

        launcher = new ClientLauncher();

        server.preSeedPlayerPosition("E2EBot");
    }
    ...
    @Test
    void alphaEnvironmentCheckPasses() throws Exception {
        String agentJar = findAgentJar();
        assertNotNull(agentJar, "rd-e2e-agent fat JAR not found. Run ./gradlew :rd-e2e-agent:fatJar first.");

        Process client = launcher.launchAlpha1017(agentJar, server.getPort(),
                statusDir, display.getDisplay(), "environment_check");
        ...
        try {
            ...
            new ScreenshotBaselineVerifier("alpha1017", "environment_check")
                    .verifyAll(statusDir, "environment_check.png");
        } finally {
            if (client.isAlive()) {
