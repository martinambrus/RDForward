...
/**
 * Release 1.2.5: Creative block palette test.
 * Opens creative inventory, grabs items from successive slots,
 * places them in a row on the ground, and captures a screenshot.
 *
 * Run with: ./gradlew :rd-e2e:test -Pe2e
 */
class Release1_2_5CreativeBlockPaletteTest {
    ...
    static void setUp() throws Exception {
        ...
        statusDir = Files.createTempDirectory("e2e-release_1_2_5-palette-").toFile();
    ...
    @Test
    void release_1_2_5CreativeBlockPalettePasses() throws Exception {
        String agentJar = findAgentJar();
        assertNotNull(agentJar, "rd-e2e-agent fat JAR not found. Run ./gradlew :rd-e2e-agent:fatJar first.");

        Process client = launcher.launchRelease1_2_5(agentJar, server.getPort(),
        ...
        try {
            ...
            new ScreenshotBaselineVerifier("release_1_2_5", "creative_block_palette")
