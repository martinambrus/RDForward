...
/**
 * Release 1.16.1: Creative block palette test. Opens creative inventory, grabs items from successive slots, places them in a row on the ground, and captures a screenshot.
 *
 * Run with: ./gradlew :rd-e2e:test -Pe2e
 */
class Release1161CreativeBlockPaletteTest {
    ...
    static void setUp() throws Exception {
        ...
        statusDir = Files.createTempDirectory("e2e-release1161-palette-").toFile();
    ...
    @Test
    void release1161CreativeBlockPalettePasses() throws Exception {
        String agentJar = findAgentJar();
        assertNotNull(agentJar, "rd-e2e-agent fat JAR not found. Run ./gradlew :rd-e2e-agent:fatJar first.");

        Process client = launcher.launchRelease1161(agentJar, server.getPort(),
        ...
        try {
            ...
            new ScreenshotBaselineVerifier("release1161", "creative_block_palette")
