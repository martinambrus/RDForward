...
/**
 * E2E test: Alpha 1.1.0 client logs into RDForward, loads the world,
 * and verifies the agent reaches COMPLETE state.
 *
 * Run with: ./gradlew :rd-e2e:test -Pe2e --tests "*Alpha110*"
 */
class Alpha110LoginScreenshotTest {
    ...
    static void setUp() throws Exception {
        ...
        statusDir = Files.createTempDirectory("e2e-alpha110-").toFile();
        statusDir.deleteOnExit();

        launcher = new ClientLauncher();

        // Pre-1.2.0 clients get kicked on first connect (TimSort warning).
        // Pre-seed the player position so the server recognizes the username.
    ...
    void alphaClientLoadsWorldAndScreenshotMatchesBaseline() throws Exception {
        ...
        try {
            ...
            new ScreenshotBaselineVerifier("alpha110", "world_loaded")
