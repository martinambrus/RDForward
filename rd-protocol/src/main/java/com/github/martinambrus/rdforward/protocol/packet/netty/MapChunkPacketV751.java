...
/**
 * 1.16.2 Play state, S2C packet 0x20: Chunk Data.
 *
 * Same as v735 but:
 * - ignoreOldLightData boolean REMOVED (was after fullChunk)
 * - Biomes changed from raw int[1024] to VarInt-length-prefixed VarInt array
 *
 * Wire format:
 *   [int]       chunkX
 *   [int]       chunkZ
 ...
 *   [VarInt]    biomesLength (if fullChunk)
 *   [VarInt[]]  biomes (if fullChunk)
 *   [VarInt]    dataSize
 *   [byte[]]    data (section data only, no biomes, no light)
 *   [VarInt]    blockEntityCount (0)
 */
public class MapChunkPacketV751 implements Packet {
    ...
    public MapChunkPacketV751() {}

    public MapChunkPacketV751(int chunkX, int chunkZ, boolean fullChunk,
                               int primaryBitMask,
    ...
    public void write(ByteBuf buf) {
        ...
        // NO ignoreOldLightData boolean (removed in 1.16.2)
        McDataTypes.writeVarInt(buf, primaryBitMask);

        // Write heightmaps as NBT compound
        writeHeightmapsNbt(buf);

        // Biomes: VarInt-length-prefixed VarInt array (changed from raw int[])
    ...
    public void read(ByteBuf buf) {
        ...
        // NO ignoreOldLightData (removed in 1.16.2)
